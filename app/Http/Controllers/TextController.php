...
class TextController extends Controller
{
    ...
    public function handle()
    {
        ...
        if (count($segmentedNotification) > 0) {
            ...
            $notification = Notification::whereRaw("DATE_FORMAT(datetime,'%Y-%m-%d %H:%i') <= '$currentDate' and is_send_notification = 0")
                ->update(['is_send_notification' => 1]);

            $notification = SegmentedNotification::whereRaw("DATE_FORMAT(datetime,'%Y-%m-%d %H:%i') <= '$currentDate' and is_send_notification = 0")
                ->update(['is_send_notification' => 1]);
